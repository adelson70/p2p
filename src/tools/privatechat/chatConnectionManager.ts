...
export class ChatConnectionManager {
  ...
  private roomCode = '';
  private sessionId = '';
  private chatRole: ChatRole = 'host';
  ...
  private ensurePc(): RTCPeerConnection {
    ...
    const handlers: PeerHandlers = {
      ...
      onDataChannel: (channel) => {
        if (channel.label === CHAT_DATA_CHANNEL) {
  ...
  async startAsHost(): Promise<SignalingPacket> {
    ...
    const channel = createChatDataChannel(pc);
    this.attachDataChannel(channel);
  ...
  getSessionMeta() {
    return {
      sessionId: this.sessionId,
      roomCode: this.roomCode,
      role: this.chatRole === 'host' ? ('send' as const) : ('receive' as const),
      chatRole: this.chatRole,
    };
  }
